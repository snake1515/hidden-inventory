...
def get_current_user():
    """Retorna el nombre del usuario logueado (admin o usuario normal)."""
...
def get_grupos_usuario():
    """Retorna la lista de grupos que el usuario puede ver. Admin ve todos."""
    if session.get('admin'):
...
def login():
    ...
    if request.method == 'POST':
        ...
        with engine.connect() as conn:
            row = conn.execute(
                text("SELECT id, nombre, password, puede_editar_almacen FROM usuarios WHERE nombre = :n AND activo = TRUE"),
                {"n": nombre}
            ).fetchone()
        if row:
...
@login_required
def index():
    puede_almacen = usuario_puede_editar_almacen()
    grupos = get_grupos_usuario()
    return render_template('index.html',
                           usuario=get_current_user(),
                           puede_editar_almacen=puede_almacen,
                           grupos=grupos)
...
def buscar():
    ...
    with engine.connect() as conn:
        if grupos is None:
            # Admin: ve todo
            df = pd.read_sql(
                text("""SELECT codigo, nombre, grupo,
            ...
            df = pd.read_sql(
                text(f"""SELECT codigo, nombre, grupo,
                                existencias_bodega, existencias_almacen,
                                ultima_mod_cantidad, ultima_mod_nombre, modificado_por
                         FROM inventario
                         WHERE (lower(nombre) LIKE :q OR lower(codigo::text) LIKE :q)
...
def admin_panel():
    ...
    with engine.connect() as conn:
        ...
        usuarios_rows = conn.execute(text("""
            SELECT u.id, u.nombre, u.puede_editar_almacen, u.activo,
                   STRING_AGG(ug.grupo, ', ' ORDER BY ug.grupo) as grupos
            FROM usuarios u
            LEFT JOIN usuario_grupos ug ON u.id = ug.usuario_id
            GROUP BY u.id, u.nombre, u.puede_editar_almacen, u.activo
            ORDER BY u.nombre
        """)).fetchall()
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(text("""
                INSERT INTO usuarios (nombre, password, puede_editar_almacen, activo)
                VALUES (:n, :p, :a, TRUE) RETURNING id
            """), {"n": nombre, "p": hashed, "a": puede_almacen})
...
def editar_usuario():
    ...
    try:
        with engine.connect() as conn:
            conn.execute(text("""
                UPDATE usuarios SET puede_editar_almacen = :a, activo = :activo WHERE id = :id
            """), {"a": puede_almacen, "activo": activo, "id": uid})
...
# ─────────────────────────────────────────
# ADMIN — INVENTARIO (agregar / borrar / CSV)
# ─────────────────────────────────────────
...
def agregar_item():
    ...
    try:
        with engine.connect() as conn:
            conn.execute(text("""
                INSERT INTO inventario (codigo, nombre, grupo, existencias_bodega, existencias_almacen,
                                        ultima_mod_cantidad, ultima_mod_nombre, modificado_por)
                VALUES (:codigo, :nombre, :grupo, :bodega, :almacen, :fecha, :admin, :admin)
            """), {
        ...
        with engine.connect() as conn:
            conn.execute(text("""
                ...
                SET codigo = :codigo_nuevo,
                    nombre = :nombre,
                    grupo = :grupo,
                    existencias_bodega = :bodega,
                    existencias_almacen = :almacen,
                    ultima_mod_cantidad = :fecha,
    ...
    try:
        ...
        if not required.issubset(set(df.columns)):
            return jsonify({'success': False,
                            'error': f'El CSV debe tener columnas: codigo, nombre, grupo. Encontradas: {list(df.columns)}'}), 400
        ...
        with engine.connect() as conn:
            conn.execute(text("""
                ...
                SET nombre = :nombre,
                    ...
                    modificado_por = :usuario
                WHERE codigo = :codigo
            """), {"nombre": nuevo_nombre, "fecha": fecha,
                   "usuario": ADMIN_USER, "codigo": codigo})
            conn.commit()
...
# ─────────────────────────────────────────
# ADMIN — BÚSQUEDA EN INVENTARIO COMPLETO
# ─────────────────────────────────────────
...
def admin_buscar_items():
    ...
    with engine.connect() as conn:
        df = pd.read_sql(
            text("""SELECT codigo, nombre, grupo, existencias_bodega, existencias_almacen,
