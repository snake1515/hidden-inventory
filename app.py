    ...
    try:
        ...
        with engine.connect() as conn:
            for row in filas:
                ...
                if referencia.lower() in ('nan', 'none'):
                    referencia = ''
                try:
                    bodega_raw = str(row.get('existencias_bodega', '0')).strip()
                    almacen_raw = str(row.get('existencias_almacen', '0')).strip()
                    bodega = int(float(bodega_raw)) if bodega_raw and bodega_raw.lower() not in ('nan','none','') else 0
                    almacen = int(float(almacen_raw)) if almacen_raw and almacen_raw.lower() not in ('nan','none','') else 0
                except:
                    bodega = 0
                    almacen = 0
                result = conn.execute(text("""
                ...
                """), {
                    "codigo": codigo, "nombre": nombre, "referencia": referencia,
...
def carga_csv_referencias_lote():
    ...
    try:
        data = request.json
        filas = data.get('filas', [])  # [{"codigo": "1", "referencia": "A412"}, ...]
        fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        actualizados = 0
        omitidos = 0
        with engine.connect() as conn:
