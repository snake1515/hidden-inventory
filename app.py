...
def now_col():
    """Fecha/hora actual en zona horaria Colombia (UTC-5, sin DST)."""
    return datetime.utcnow() - timedelta(hours=5)
from sqlalchemy import create_engine, text
import bcrypt
import httpx
from pdf_movimientos import generar_pdf_ingreso, generar_pdf_egreso, generar_pdf_traslado, generar_pdf_csv
...
def usuario_puede_crear_movimientos():
    if session.get('admin'):
        return True
    return session.get('puede_crear_movimientos', False)
...
def subir_pdf_supabase(file_bytes: bytes, filename: str, content_type: str = "application/pdf") -> str | None:
    """Sube un archivo al bucket y devuelve el path almacenado, o None si falla."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        return None
    path = f"{now_col().strftime('%Y/%m')}/{filename}"
...
def url_firmada_supabase(path: str, expiry: int = 31536000) -> str | None:
    """Genera una URL firmada de Supabase Storage válida por `expiry` segundos."""
    if not SUPABASE_URL or not SUPABASE_KEY:
        return None
    url = f"{SUPABASE_URL}/storage/v1/object/sign/{STORAGE_BUCKET}/{path}"
    r = httpx.post(url, json={"expiresIn": expiry}, headers=_storage_headers(), timeout=10)
    if r.status_code == 200:
        signed = r.json().get("signedURL") or r.json().get("signedUrl")
        if signed:
            return signed if signed.startswith("http") else f"{SUPABASE_URL}{signed}"
    return None
...
def login():
    ...
    if request.method == 'POST':
        ...
        with engine.connect() as conn:
            row = conn.execute(
                text("SELECT id, nombre, password, puede_editar_almacen, puede_editar_bodega, puede_ver_movimientos, puede_crear_movimientos FROM usuarios WHERE nombre = :n AND activo = TRUE"),
                {"n": nombre}
            ).fetchone()
        if row:
            if bcrypt.checkpw(password.encode(), row.password.encode()):
                ...
                session['puede_crear_movimientos'] = row.puede_crear_movimientos
...
def actualizar():
    ...
    data    = request.json
    usuario = get_current_user()
    fecha   = now_col().strftime("%Y-%m-%d %H:%M:%S")
    try:
        with engine.connect() as conn:
            conn.execute(text("""
...
def actualizar_referencia():
    if not (usuario_puede_editar_bodega() or usuario_puede_editar_almacen()):
        return jsonify({'success': False, 'error': 'Sin permiso para editar'}), 403
    data    = request.json
    usuario = get_current_user()
    fecha   = now_col().strftime("%Y-%m-%d %H:%M:%S")
    try:
...
def movimientos_lista():
    ...
    conditions = ["1=1", "m.mov_padre_id IS NULL"]
    params     = {}
    if tipo:
    ...
    sql = text(f"""
        SELECT m.id, m.tipo, m.consecutivo, m.documento, m.comentario,
               ...
               COALESCE(
                   (SELECT COUNT(*) FROM movimiento_items mi2
                    JOIN movimientos hijos ON hijos.id = mi2.movimiento_id
                    WHERE hijos.id = m.id OR hijos.mov_padre_id = m.id),
                   0
               ) as num_items,
               COALESCE(
                   (SELECT SUM(mi2.cantidad) FROM movimiento_items mi2
                    JOIN movimientos hijos ON hijos.id = mi2.movimiento_id
                    WHERE hijos.id = m.id OR hijos.mov_padre_id = m.id),
                   0
               ) as total_unidades
    ...
    with engine.connect() as conn:
        mov = conn.execute(text("""
            SELECT id, tipo, consecutivo, documento, comentario,
                   ...
                   motivo_egreso, cliente, realizado_por, fecha, mov_padre_id
            FROM movimientos WHERE id = :id
        """), {"id": mov_id}).fetchone()
        if not mov:
            return jsonify({'error': 'No encontrado'}), 404

        # Items propios del movimiento (vacío si es un padre CSV sin items directos)
        ...
        # Si es un padre CSV, traer también los items de TODOS sus lotes hijos
        # y el listado de lotes para mostrarlos como sub-secciones.
        lotes = []
        if mov.tipo == 'csv' and mov.mov_padre_id is None:
            lotes_rows = conn.execute(text("""
                SELECT id, consecutivo, documento, comentario, fecha
                FROM movimientos WHERE mov_padre_id = :id
                ORDER BY consecutivo ASC
            """), {"id": mov_id}).fetchall()
            for lote in lotes_rows:
                items_lote = conn.execute(text("""
                    SELECT producto_codigo, producto_nombre, cantidad, ubicacion_destino
                    FROM movimiento_items WHERE movimiento_id = :id
                    ORDER BY id
                """), {"id": lote.id}).fetchall()
                lotes.append({
                    ...
                    "fecha": str(lote.fecha),
                    "items": [dict(i._mapping) for i in items_lote],
                })
...
def movimientos_eliminar(mov_id):
    ...
    try:
        with engine.connect() as conn:
            ...
            fecha_str = now_col().strftime("%Y-%m-%d %H:%M:%S")
            usuario = get_current_user()
...
def _siguiente_consecutivo(conn, tipo: str) -> str:
    ...
    prefijos = {'ingreso': 'ING', 'egreso': 'EGR', 'traslado': 'TRL', 'csv': 'CSV'}
...
def _letra_lote(indice: int) -> str:
...
def movimientos_crear():
    if not usuario_puede_crear_movimientos():
        return jsonify({'success': False, 'error': 'No tienes permiso para crear movimientos'}), 403
    data    = request.json
    usuario = get_current_user()
            ...
            row = conn.execute(text("""
                ...
                VALUES
                    (:tipo, :consec, :doc, :com, :pdf,
                     :prov, :nfac, :fdoc, :frec,
                     :motivo, :cliente, :usuario, :fecha_col)
                RETURNING id
            """), {
                "tipo":   tipo,
                "consec": consecutivo,
                ...
                items_pdf = [
                    {
                        ...
                        "referencia":  str(i.get("referencia") or ""),
...
def movimientos_upload_pdf():
    ...
    if ext not in ALLOWED_EXTENSIONS:
        return jsonify({'success': False, 'error': f'Tipo de archivo no permitido ({ext})'}), 400
    content_type = ALLOWED_EXTENSIONS[ext]
    ts       = now_col().strftime('%Y%m%d_%H%M%S')
    filename = f"{ts}_{f.filename}"
    path     = subir_pdf_supabase(f.read(), filename, content_type)
    if path:
        return jsonify({'success': True, 'path': path})
    return jsonify({'success': False, 'error': 'Error subiendo archivo. Verifica SUPABASE_URL y SUPABASE_KEY.'}), 500
...
@app.route('/kardex/buscar_productos')
@movimientos_required
def kardex_buscar_productos():
    """Buscador multi-palabra igual al de inventario/movimientos."""
    texto = request.args.get('q', '').lower().strip()
    if not texto:
        return jsonify([])
    q        = f"%{texto}%"
...
@app.route('/kardex/lista_productos')
@movimientos_required
def kardex_lista_productos():
    """Vista general: todos los productos con su último movimiento (para cuando no se busca nada)."""
    with engine.connect() as conn:
        df = pd.read_sql(text("""
            SELECT i.codigo, i.nombre, i.grupo,
                   i.existencias_bodega, i.existencias_almacen,
                   ult.tipo as ultimo_tipo, ult.consecutivo as ultimo_consecutivo,
                   ult.fecha as ultima_fecha
            FROM inventario i
            LEFT JOIN LATERAL (
                SELECT m.tipo, m.consecutivo, m.fecha
                FROM movimiento_items mi
                JOIN movimientos m ON m.id = mi.movimiento_id
                WHERE mi.producto_codigo = i.codigo::text
                ORDER BY m.fecha DESC
                LIMIT 1
            ) ult ON true
            ...
            LIMIT 500
        """), conn)
    df['ultima_fecha'] = df['ultima_fecha'].astype(str).replace('NaT', None).replace('None', None)
    return jsonify(df.to_dict(orient='records'))

@app.route('/kardex/historial/<codigo>')
@movimientos_required
def kardex_historial(codigo):
    """Historial completo de movimientos de un producto específico."""
    with engine.connect() as conn:
        producto = conn.execute(text("""
            SELECT codigo, nombre, referencia, marca, grupo,
                   existencias_bodega, existencias_almacen
            FROM inventario WHERE codigo::text = :codigo
        """), {"codigo": str(codigo)}).fetchone()

        if not producto:
            return jsonify({'error': 'Producto no encontrado'}), 404

        movs = conn.execute(text("""
            SELECT m.id, m.tipo, m.consecutivo, m.realizado_por, m.fecha,
                   m.proveedor, m.motivo_egreso, m.documento,
                   mi.cantidad, mi.ubicacion_origen, mi.ubicacion_destino
            FROM movimiento_items mi
            JOIN movimientos m ON m.id = mi.movimiento_id
            WHERE mi.producto_codigo = :codigo
            ORDER BY m.fecha DESC
            LIMIT 300
        """), {"codigo": str(codigo)}).fetchall()

    prod_d = dict(producto._mapping)
    historial = []
    for m in movs:
        d = dict(m._mapping)
        d['fecha'] = str(d['fecha'])
    ...
    return jsonify({'producto': prod_d, 'historial': historial})
    ...
    with engine.connect() as conn:
        ...
        usuarios_rows = conn.execute(text("""
            SELECT u.id, u.nombre, u.puede_editar_almacen, u.puede_editar_bodega,
                   u.activo, u.puede_ver_movimientos, u.puede_crear_movimientos,
        ...
        with engine.connect() as conn:
            result = conn.execute(text("""
                INSERT INTO usuarios (nombre, password, puede_editar_almacen, puede_editar_bodega,
                                      puede_ver_movimientos, puede_crear_movimientos, activo)
                VALUES (:n, :p, :a, :b, :mov, :crear_mov, TRUE) RETURNING id
            """), {"n": data['nombre'].strip(), "p": hashed,
                   ...
                   "crear_mov": data.get('puede_crear_movimientos', False)})
        ...
        with engine.connect() as conn:
            conn.execute(text("""
                UPDATE usuarios SET puede_editar_almacen=:a, puede_editar_bodega=:b,
                    puede_ver_movimientos=:mov, puede_crear_movimientos=:crear_mov,
...
def purgar_inventario():
    ...
    try:
        with engine.connect() as conn:
            ...
            # Reinicio total: la purga también borra todo el historial de
            # movimientos (huérfano sin el inventario) y reinicia consecutivos.
            conn.execute(text("DELETE FROM movimiento_items"))
            conn.execute(text("DELETE FROM movimientos"))
            conn.execute(text("UPDATE movimientos_consecutivos SET ultimo = 0"))
            conn.commit()
    ...
    try:
        with engine.connect() as conn:
            ...
            """), {
                ...
                "fecha": now_col().strftime("%Y-%m-%d %H:%M:%S"), "admin": ADMIN_USER
            })
            conn.commit()
        return jsonify({'success': True})
    except Exception as e:
...
# ─────────────────────────────────────────
# ADMIN — CSV LOTES (carga padre + lotes hijos)
# ─────────────────────────────────────────
@app.route('/admin/iniciar_carga_csv', methods=['POST'])
def iniciar_carga_csv():
    """Crea el movimiento PADRE (ej. CSV-0007) que agrupará todos los lotes
    de esta carga. Se llama una sola vez al inicio, antes de subir lotes."""
    if not session.get('admin'):
        return jsonify({'success': False, 'error': 'No autorizado'}), 403
    try:
        nombre_archivo = request.json.get('nombre_archivo', '') or 'archivo.csv'
        usuario = get_current_user()
        with engine.connect() as conn:
            consecutivo = _siguiente_consecutivo(conn, 'csv')
            row_mov = conn.execute(text("""
                INSERT INTO movimientos
                    (tipo, consecutivo, documento, comentario, realizado_por, fecha,
                     agregados, rechazados, estado_carga)
                VALUES
                    (:tipo, :consec, :doc, :com, :usuario, :fecha_col, '[]'::jsonb, '[]'::jsonb, 'en_progreso')
                RETURNING id
            """), {
                "tipo": "csv",
                "consec": consecutivo,
                "doc": nombre_archivo,
                "com": "Carga masiva en progreso...",
                "usuario": usuario,
                "fecha_col": now_col(),
            })
            mov_padre_id = row_mov.fetchone().id
            conn.commit()
        return jsonify({'success': True, 'mov_padre_id': mov_padre_id, 'consecutivo': consecutivo})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def carga_csv_lote():
    """Procesa UN lote de filas. Crea un movimiento hijo (ej. CSV-0007-A)
    con los items reales que afectan inventario, y acumula agregados/
    rechazados en el movimiento padre para el PDF final."""
    if not session.get('admin'):
        return jsonify({'success': False, 'error': 'No autorizado'}), 403
    try:
        filas = request.json.get('filas', [])
        mov_padre_id = request.json.get('mov_padre_id')
        indice_lote  = request.json.get('indice_lote', 0)
        if not mov_padre_id:
            return jsonify({'success': False, 'error': 'Falta mov_padre_id — llama primero a /admin/iniciar_carga_csv'}), 400
        ...
        with engine.connect() as conn:
            padre = conn.execute(text("""
                SELECT consecutivo FROM movimientos WHERE id = :id
            """), {"id": mov_padre_id}).fetchone()
            if not padre:
                return jsonify({'success': False, 'error': 'Movimiento padre no encontrado'}), 404
            consecutivo_padre = padre.consecutivo
            consecutivo_hijo  = f"{consecutivo_padre}-{_letra_lote(indice_lote)}"
            ...
            for row in filas:
                try:
                    ...
                    if not codigo or not nombre or not grupo:
                        faltan = []
                        if not codigo: faltan.append('código')
                        if not nombre: faltan.append('nombre')
                        if not grupo:  faltan.append('grupo')
                        omitidos += 1
                        rechazados_lote.append({"fila": row, "motivo": f"Falta(n): {', '.join(faltan)}"})
                        continue
                    ...
                    if result.rowcount > 0:
                        insertados += 1
                        agregados_lote.append({
                            "codigo": codigo, "nombre": nombre, "grupo": grupo,
                            "existencias_bodega": bodega, "existencias_almacen": almacen
                        })
                    else:
                        omitidos += 1
                        rechazados_lote.append({"fila": row, "motivo": "El código ya existe en el inventario"})

                except Exception as fila_err:
                    omitidos += 1
                    rechazados_lote.append({"fila": row, "motivo": f"Error procesando la fila: {fila_err}"})
                    continue

            # ── Movimiento HIJO con los items reales (afecta inventario/kardex) ──
            row_hijo = conn.execute(text("""
                INSERT INTO movimientos
                    (tipo, consecutivo, documento, comentario, realizado_por, fecha,
                     mov_padre_id, estado_carga)
                VALUES
                    (:tipo, :consec, :doc, :com, :usuario, :fecha_col, :padre_id, 'completo')
                RETURNING id
            """), {
                "tipo": "csv",
                "consec": consecutivo_hijo,
                "doc": f"Lote {_letra_lote(indice_lote)}",
                "com": f"{insertados} agregados, {omitidos} rechazados",
                "usuario": usuario,
                "padre_id": mov_padre_id,
                "fecha_col": now_col(),
            })
            mov_hijo_id = row_hijo.fetchone().id

            for p in agregados_lote:
                total_cant = p["existencias_bodega"] + p["existencias_almacen"]
                destino = "bodega" if p["existencias_bodega"] >= p["existencias_almacen"] else "almacen"
                conn.execute(text("""
                    INSERT INTO movimiento_items
                        (movimiento_id, producto_codigo, producto_nombre, cantidad, ubicacion_destino)
                    VALUES (:mid, :cod, :nom, :cant, :dest)
                """), {
                    "mid": mov_hijo_id, "cod": p["codigo"], "nom": p["nombre"],
                    "cant": max(total_cant, 0), "dest": destino
                })

            # ── Acumular en el padre (para el PDF final) ──
            conn.execute(text("""
                UPDATE movimientos
                SET agregados  = agregados  || CAST(:nuevos_agregados  AS jsonb),
                    rechazados = rechazados || CAST(:nuevos_rechazados AS jsonb)
                WHERE id = :padre_id
            """), {
                "nuevos_agregados":  json.dumps(agregados_lote,  ensure_ascii=False, default=str),
                "nuevos_rechazados": json.dumps(rechazados_lote, ensure_ascii=False, default=str),
                "padre_id": mov_padre_id,
            })
        ...
        return jsonify({
            ...
            'omitidos': omitidos,
            'consecutivo_lote': consecutivo_hijo,
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/admin/finalizar_carga_csv', methods=['POST'])
def finalizar_carga_csv():
    """Se llama una vez al terminar todos los lotes. Genera el PDF final
    con TODOS los agregados/rechazados acumulados de la carga completa."""
    if not session.get('admin'):
        return jsonify({'success': False, 'error': 'No autorizado'}), 403
    try:
        mov_padre_id = request.json.get('mov_padre_id')
        if not mov_padre_id:
            return jsonify({'success': False, 'error': 'Falta mov_padre_id'}), 400
        ...
        with engine.connect() as conn:
            padre = conn.execute(text("""
                SELECT consecutivo, documento, agregados, rechazados
                FROM movimientos WHERE id = :id
            """), {"id": mov_padre_id}).fetchone()
            if not padre:
                return jsonify({'success': False, 'error': 'Movimiento padre no encontrado'}), 404

            # Supabase puede devolver columnas jsonb como string — parseamos si es necesario
            _agr = padre.agregados or []
            _rej = padre.rechazados or []
            if isinstance(_agr, str):
                _agr = json.loads(_agr)
            if isinstance(_rej, str):
                _rej = json.loads(_rej)
            agregados_total  = _agr
            rechazados_total = _rej
            total_insertados = len(agregados_total)
            total_omitidos   = len(rechazados_total)

            # Primero marcar como completo aunque falle el PDF
            conn.execute(text("""
                UPDATE movimientos
                SET comentario = :com, estado_carga = 'completo'
                WHERE id = :id
            """), {
                "com": f"Carga masiva completa: {total_insertados} agregados, {total_omitidos} rechazados",
                "id": mov_padre_id,
            })
            conn.commit()

            # Generar PDF separado — si falla, el estado ya quedó guardado
            try:
                fecha_pdf = now_col().strftime("%d/%m/%Y %I:%M %p")
                # Solo se pasan los conteos y los rechazados (no la lista completa de agregados)
                pdf_bytes = generar_pdf_csv({
                    "consecutivo":    padre.consecutivo,
                    "archivo_nombre": padre.documento,
                    "comentario":     "",
                    "realizado_por":  usuario,
                    "fecha_registro": fecha_pdf,
                    "agregados":      total_insertados,
                    "rechazados":     rechazados_total,
                })
                filename = f"{padre.consecutivo}.pdf"
                pdf_generado_path = subir_pdf_supabase(pdf_bytes, filename)
                if pdf_generado_path:
                    with engine.connect() as conn2:
                        conn2.execute(text("""
                            UPDATE movimientos SET pdf_generado = :pdf WHERE id = :id
                        """), {"pdf": pdf_generado_path, "id": mov_padre_id})
                        conn2.commit()
            except Exception as pdf_err:
                import traceback
                print(f"[PDF CSV] Error generando PDF: {pdf_err}")
                print(traceback.format_exc())
...
@app.route('/admin/carga_csv_nombres_lote', methods=['POST'])
def carga_csv_nombres_lote():
    """Actualiza SOLO el nombre de productos existentes a partir de un CSV
    con columnas codigo,nombre. No toca existencias, referencia, marca ni
    grupo. Si el código no existe en el inventario, la fila se omite."""
    if not session.get('admin'):
        return jsonify({'success': False, 'error': 'No autorizado'}), 403
    try:
        filas  = request.json.get('filas', [])
        ...
        with engine.connect() as conn:
            for row in filas:
                codigo       = str(row.get('codigo', '')).strip()
                nombre_nuevo = str(row.get('nombre', '')).strip()
                if not codigo or not nombre_nuevo or nombre_nuevo.lower() in ('nan', '0', 'none', ''):
                    omitidos += 1; continue
                result = conn.execute(text("""
                    UPDATE inventario SET nombre=:nombre, modificado_por=:u, ultima_mod_nombre=:f
                    WHERE codigo=:c
                """), {"nombre": nombre_nuevo, "u": ADMIN_USER, "f": fecha, "c": codigo})
                if result.rowcount > 0: actualizados += 1
                else: omitidos += 1
            conn.commit()
        return jsonify({'success': True, 'actualizados': actualizados, 'omitidos': omitidos})
...
@app.route('/admin/regenerar_pdfs_faltantes')
    ...
    with engine.connect() as conn:
        movs = conn.execute(text("""
            SELECT id, tipo, consecutivo, documento, comentario,
                   ...
                   motivo_egreso, cliente, realizado_por, fecha
            FROM movimientos
            WHERE pdf_generado IS NULL
              AND tipo IN ('ingreso','egreso','traslado')
            ORDER BY id
        """)).fetchall()
        ...
        for m in movs:
            try:
                items_rows = conn.execute(text("""
                    SELECT mi.producto_codigo AS codigo, mi.producto_nombre AS nombre,
                           mi.cantidad, mi.ubicacion_origen AS origen,
                           mi.ubicacion_destino AS destino, inv.referencia AS referencia
                    FROM movimiento_items mi
                    LEFT JOIN inventario inv ON inv.codigo::text = mi.producto_codigo
                    WHERE mi.movimiento_id = :mid
                """), {"mid": m.id}).fetchall()

                items_pdf = [{
                    "codigo": str(i.codigo or ""),
                    "referencia": str(i.referencia or ""),
                    "nombre": str(i.nombre or ""),
                    "cantidad": int(i.cantidad or 0),
                    "origen": str(i.origen or "bodega"),
                    "destino": str(i.destino or "bodega"),
                } for i in items_rows]

                fecha_pdf = m.fecha.strftime("%d/%m/%Y %I:%M %p") if m.fecha else ""

                if m.tipo == 'ingreso':
                    pdf_bytes = generar_pdf_ingreso({
                        "consecutivo": m.consecutivo,
                        "proveedor": m.proveedor or '',
                        "numero_factura": m.numero_factura or '',
                        "fecha_documento": m.fecha_documento,
                        "fecha_recepcion": m.fecha_recepcion,
                        "documento_soporte": m.documento or '',
                    ...
                    pdf_bytes = generar_pdf_egreso({
                        "consecutivo": m.consecutivo,
                        "motivo_egreso": m.motivo_egreso or '',
                        "cliente": m.cliente or '',
                        "documento_soporte": m.documento or '',
                        "comentario": m.comentario or '',
                        "realizado_por": m.realizado_por,
                        "fecha_registro": fecha_pdf,
                ...
                filename = f"{m.consecutivo}.pdf"
                path = subir_pdf_supabase(pdf_bytes, filename)
                if path:
                    conn.execute(text("UPDATE movimientos SET pdf_generado = :p WHERE id = :id"),
                                 {"p": path, "id": m.id})
                    conn.commit()
                    resultados.append({"id": m.id, "consecutivo": m.consecutivo, "status": "ok"})
                else:
                    resultados.append({"id": m.id, "consecutivo": m.consecutivo, "status": "error_subida"})
            except Exception as e:
                conn.rollback()
                resultados.append({"id": m.id, "consecutivo": m.consecutivo, "status": f"error: {e}"})
