...
def buscar():
    ...
    base_select = """SELECT codigo, nombre, referencia, marca, grupo,
                     ...
                     FROM inventario"""
    base_where = """WHERE (lower(nombre) LIKE :q OR lower(codigo::text) LIKE :q
                       OR lower(coalesce(referencia,'')) LIKE :q
                       OR lower(coalesce(marca,'')) LIKE :q)"""

    with engine.connect() as conn:
        if grupos is None:
            df = pd.read_sql(text(f"{base_select} {base_where} ORDER BY nombre ASC LIMIT 200"),
                             conn, params={"q": q})
        else:
            if not grupos:
                return jsonify([])
            placeholders = ','.join([f"'{g}'" for g in grupos])
            df = pd.read_sql(text(f"{base_select} {base_where} AND grupo IN ({placeholders}) ORDER BY nombre ASC LIMIT 200"),
                             conn, params={"q": q})
    df['rowid'] = df['codigo']
